...
st.set_page_config(page_title="ЛИЦЕНЗИИИИИИ", page_icon="🔑", layout="centered")
...
with tab1:
    ...
    if submit_button:
        if not user_name or not user_id or not mine_name:
            st.error("Ошибка! Все текстовые поля должны быть заполнены дурик!.")
        else:
            ...
            try:
                ...
                st.success("Данные успешно внесены в таблицу! Молодец!")
